"use client"
import style from './style.module.css'
import React, { useState } from "react";
import { useEffect } from "react";
// import { useRef } from "react";
import axios from '@/utils/axios';
import { useRef } from 'react';
import { useRouter } from "next/navigation";
import {AiFillGoogleCircle} from "react-icons/ai"
import {RxCross2} from "react-icons/rx"
import { asyncaddemployee , asynccurrentemployee } from "@/store/Actions/employeeAction"
import Link from 'next/link';
import { useSelector, useDispatch } from "react-redux";

const index = () => {
   
  const { employee, isAuthenticated} = useSelector((state) => state.employeeReducer);
  
  const dispatch = useDispatch();
  const pt = useRef(null)
  const router = useRouter();



  const input = {
    // height: '60vh',.
    width:'25vw',
    padding:'.6vmax 0vmax',
    paddingLeft:'1.4vmax',
    borderRadius:'3vmax',
    outline:'none',
    focus:'none',
    boxShadow: ' 0 1px 1px rgba(0,0,0,0.12), 0 2px 2px rgba(0,0,0,0.12)'

  };

  const button = {
    // height: '60vh',.
    padding:'.6vmax 0vmax',
    borderRadius:'3vmax',
    outline:'none',
    focus:'none',
    // backgroundColor:'#f2d069',
    backgroundColor:'#1e779d',
    border:'none',
    color:'#fff',
    fontFamily:'roobert',
    letterSpacing:'1px',
    fontWeight:'500',
    boxShadow: '0 3px 6px rgba(0,0,0,0.16), 0 3px 6px rgba(0,0,0,0.23)'
  };

    const initialFormData = {
      name:'',
      email: '',
      password: '',
      contact:''
      // Add more fields as needed
    };
    const [formData, setformData] =  useState(initialFormData)

    const inputEvent=(event)=>{
      const {name, value} = event.target;

      setformData((preData)=>{
          return {
              ...preData,
              [name]:value,
          }
      })
    }
   
    const formHandler = async (e)=>{
        e.preventDefault();
        dispatch(asyncaddemployee(formData))
        setformData(initialFormData); // Reset form fields
        router.push('/employee/confirmp');     
    }

    const rempl = ()=>{
       router.push("/student/register");
    }
    
  return (
    
    <>
       <div className={style.register_page}>
            <Link style={{color:"#111"}} href="/"><RxCross2 className={style.crosss} /></Link>
           <div className={style.register}>
            
               <div className={style.left_l}>
                    <h3>INFORMATION</h3>
                    <p> Your journey to knowledge begins here. Sign up to access a wide range of courses and resources.</p>
                    <div className={style.lbtm}>
                         <img src="data:image/jpeg;base64," alt="" />
                    </div>
               </div>
               <div className={style.right_r}>
                   <h3>REGISTER FORM</h3>
                   <div className={style.options}>
                          <small>As a</small>
                          <div className={style.se}>
                              <h6 onClick={rempl} >Student</h6>
                              <h6 ref={pt} className={style.h6} >Employee</h6>
                          </div>
                     </div>
                     <form className="flex flex-col items-center mt-1" onSubmit={formHandler}>
                        <div className="mb-3">
                          <input style={input}  placeholder='Enter Comapny Email' required name='email' value={formData.email} onChange={inputEvent} type="email" className="form-control" id="text" aria-describedby="emailHelp" autoComplete="username" />
                        </div>
                        <div className="mb-3">
                          <input style={input}  placeholder='Enter Full Name' required name='name' value={formData.name} onChange={inputEvent} type="text" className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" autoComplete="username" />
                        </div>
                        <div className="mb-3">
                          <input style={input}  placeholder='Enter Contact Number' required name='contact' value={formData.contact} onChange={inputEvent} type="text" className="form-control" id="email" aria-describedby="emailHelp" autoComplete="username" />
                        </div>
                        
                        <div className="mb-3">
                          <input style={input}  required name='password' value={formData.password} onChange={inputEvent} placeholder='password' type="password" className="form-control" id="exampleInputPassword1" autoComplete="current-password" />
                        </div>
                        
                        <button style={button} onClick={formHandler} type="submit" className="btn btn-primary text-center flex items-center justify-center w-48 mt-3 bg-yellow-500 ">Sign Up</button>
                     </form>
                     <div className={style.or}>
                         <div className={style.line}></div>
                         <p>Or</p>
                         <div className={style.line}></div>
                     </div>
                     <div className={style.dvg}>
                        <Link href="/student/login" style={{textDecoration:"none",color:"#111",margin:"initial"}}>
                            <div className={style.ggl}>
                            <h6>Already Have An Account</h6>
                            </div>
                        </Link>
                        <div className={style.ggl}>
                          <h6>Login with Google</h6>
                          <AiFillGoogleCircle className=' text-2xl' />
                        </div>
                     </div>
                     
                    
                    
               </div>
           </div>
       </div>
    </>
    
  )
}

export default index